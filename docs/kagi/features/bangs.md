...
By default, if no regex is specified, a regex pattern of `^(\w+)(?:\s*(\w+)\s*(.*))?` is used. With this, `$1` and `$2` are the first and second words in the query, and `$3` is the remainder. For example, with no regex pattern, a template of `https://translate.kagi.com/?from=$1&to=$2&q=$3` with a query of `english french where is the bathroom` will redirect to `https://translate.kagi.com/?from=english&to=french&q=where+is+the+bathroom`.
