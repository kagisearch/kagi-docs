[Welcome](welcome.md)

---

#  About

- [Company](./company/index.md)
	- [About](./company/index.md)
	- [Products](./company/products.md)
	- [History](./company/history.md)
	- [Name](./company/meaning.md)
	- [Mascot](./company/mascot.md)
	- [Logo](./company/logo.md)
	- [Assets](./company/assets.md)
	- [Jobs](./company/hiring-kagi.md)
		- [Work on Kagi Search](./company/hiring-kagi.md)
		- [Work on Orion Browser](./company/hiring-orion.md)
	- [Contact Us](./company/contact.md)

- [Support & Community](./support-and-community/troubleshooting-webpage-issues.md)
	- [Troubleshooting Webpage Issues](./support-and-community/troubleshooting-webpage-issues.md)
	- [Supported OS Versions](./support-and-community/os-versions.md)
	- [Product Roadmap & Feedback Forum](./support-and-community/roadmap-feedback-forum.md)
	- [Discord Server](./support-and-community/discord-server.md)
	- [Email Support](./support-and-community/email-support.md)
	- [macOS Keyboard Shortcuts](./support-and-community/keyboard-shortcuts.md)
	- [Fastmail Issues](./support-and-community/fastmail-issues.md)

# User Guide

- [Getting Started](./getting-started/quick-start.md)
	- [Quick Start](./getting-started/quick-start.md)
	- [Frequently Asked Questions](./getting-started/faqs.md)
	- [Installing Orion](./getting-started/installing-orion.md)
	- [Importing Data From Other Browsers](./getting-started/importing.md)
	- [Default Search Engine](./getting-started/search-engine.md)
	- [Orion is in Public Beta](./getting-started/public-beta.md)

- [Orion+](./orion-plus/orion-plus.md)

- [Features](./features/website-settings.md)
	- [Website Settings](./features/website-settings.md)
	- [Password Management](./features/password-management.md)
	- [Syncing Data](./features/syncing-data.md)
	- [Reader Mode](./features/reader-mode.md)
	- [Vertical Tabs](./features/vertical-tabs.md)
	- [Tab Groups](./features/tab-groups.md)
	- [Quick Searches](./features/quick-searches.md)
	- [Viewing Web Archives](./features/web-archives.md)
	- [Picture-in-Picture Videos](./features/pip.md)
	- [Focus Mode](./features/focus-mode.md)
	- [Low Power Mode](./features/low-power-mode.md)
	- [Allow Copy Paste](./features/allow-copy-paste.md)
	- [Edit Text on Page](./features/edit-text-on-page.md)
	- [Float on Top](./features/float-on-top.md)
	- [Show Console Errors](./features/show-errors.md)
	- [Block Auto-Play](./features/block-auto-play.md)

- [Browser Extensions](./browser-extensions/macos-extensions.md)
	- [macOS Extensions](./browser-extensions/macos-extensions.md)
	- [uBlock Origin on macOS](./browser-extensions/ublock-origin.md)
	- [iOS & iPadOS Extensions](./browser-extensions/ios-ipados-extensions.md)
	- [1Password](./browser-extensions/1password.md)

- [Privacy & Security](./privacy-and-security/ad-tracking-blocking.md)
	- [Configure Ad & Tracking Blocking](./privacy-and-security/ad-tracking-blocking.md)
	- [Respecting Privacy](./privacy-and-security/respecting-privacy.md)
	- [Protecting Privacy](./privacy-and-security/protecting-privacy.md)
	- [Preventing Fingerprinting](./privacy-and-security/preventing-fingerprinting.md)

- [Why Orion?](./why-orion/orion-vs-safari.md)
	- [Orion vs. Safari](./why-orion/orion-vs-safari.md)
	- [Orion vs. Chrome, Firefox, & Others](./why-orion/kagi-vs-others.md)
	- [Speed](./why-orion/orion-speed.md)
	- [Memory Usage](./why-orion/memory-usage.md)

- [Live stats](./misc/stats.md)

# Credits

- [Contributors](misc/contributors.md)
